...
export default function CN15Page() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это секция нагревательная СН-15
          if (!name.includes('секция нагревательная сн-15')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <Head>
        <title>СН-15 — Кабельный теплый пол , ЧТК | Elektromos</title>
        <meta name="description" content="Купить ЧТК СН-15 для тёплого пола: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:title" content="СН-15 — купить кабельный теплый пол | Elektromos" />
        <meta property="og:description" content="Купить  ЧТК СН-15 для тёплого пола: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-3xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Теплый пол под стяжку
...
<nav className="flex flex-wrap items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-400">
          ...
          <span>/</span>
          <span className="text-white">Теплый пол под стяжку
