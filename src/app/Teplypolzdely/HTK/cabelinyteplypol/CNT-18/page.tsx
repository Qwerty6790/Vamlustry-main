...
export default function CNT18Page() {
    ...
    try {
      ...
      if (data && data.products) {
        const targetProductNames = [
          'секция нагревательная снт-18',
        ];

        const filteredProducts = data.products.filter((product: any) => {
          const name = (product.name || '').toLowerCase();
          if (!name.includes('секция нагревательная снт-18')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <Head>
        <title>СНТ-18 — Кабельный теплый пол, ЧТК | Elektromos</title>
        <meta name="description" content="Купить СНТ-18 ЧТК для тёплого пола: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:title" content="СНТ-18 — купить кабельный теплый пол | Elektromos" />
        <meta property="og:description" content="Купить СНТ-18 для тёплого пола: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:url" content="https://elektromos.uz/Teplypolzdely/HTK/cabelnyteplypol/CN-15" />
        <meta property="og:image" content="/images/seris/СНТ-18.webp" />
        <meta name="twitter:card" content="summary_large_image" />
      </Head>
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-3xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Тонкие теплые пол в плиточный клей или стяжку
         ...
         <nav className="flex flex-wrap items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-400">
          ...
          <span>/</span>
          <span className="text-white">Тонкие теплые пол в плиточный клей или стяжку
