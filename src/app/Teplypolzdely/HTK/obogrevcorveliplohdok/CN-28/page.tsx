...
export default function CN28Page() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это секция нагревательная СН-28
          if (!name.includes('секция нагревательная сн-28')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <Head>
        <title>СН-28 — Обогрев кровель и площадок, ЧТК | Elektromos</title>
        <meta name="description" content="Купить  ЧТК СН-28 для обогрева кровель и площадок: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:title" content="СН-28 — купить обогрев кровли и площадок | Elektromos" />
        <meta property="og:description" content="Купить ЧТК СН-28 для обогрева кровель и площадок: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:url" content="https://elektromos.uz/Teplypolzdely/HTK/obogrevcrovliiplohadek/CN-28" />
        <meta property="og:image" content="/images/seris/СН-28.webp" />
        <meta name="twitter:card" content="summary_large_image" />
      </Head>
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-3xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Обогрев открытых площадок, ступеней, кровли
</h2>
<nav className="flex flex-wrap items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-400">
          ...
          <span>/</span>
          <span className="text-white">Обогрев открытых площадок, ступеней, кровли
