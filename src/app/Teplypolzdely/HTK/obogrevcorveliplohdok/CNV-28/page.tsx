...
export default function CNT28Page() {
    ...
    try {
      ...
      if (data && data.products) {
        const targetProductNames = [
          'секция нагревательная снв-28',
        ];

        const filteredProducts = data.products.filter((product: any) => {
          const name = (product.name || '').toLowerCase();
          if (!name.includes('секция нагревательная снв-28')) return false;
          return targetProductNames.some(targetName => name.includes(targetName));
        });
        // Собираем диапазоны площадей из name
        const areaSet = new Set<string>();
        filteredProducts.forEach((product: any) => {
          const name = (product.name || '').toLowerCase();
          // ищем паттерн типа "(\d{1,2},\d-\d{1,2},\d)" например 8,0-10,1
          const match = name.match(/(\d{1,2},\d-\d{1,2},\d)/);
          if (match && match[1]) areaSet.add(match[1]);
        });
        setAvailableAreas(Array.from(areaSet).sort((a, b) => parseFloat(a.split('-')[0].replace(',', '.')) - parseFloat(b.split('-')[0].replace(',', '.'))));
  ...
  useEffect(() => {
    ...
    const filtered = allFilteredProducts.filter((product: any) => {
      const name = (product.name || '').toLowerCase();
      return name.includes(selectedArea);
    });
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <Head>
        <title>СНВ-28 — Обогрев кровель и площадок, ЧТК | Elektromos</title>
        <meta name="description" content="Купить  ЧТК СНВ-28 для обогрева кровель и площадок: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:title" content="СНВ-28 — купить обогрев кровли и площадок | Elektromos" />
        <meta property="og:description" content="Купить ЧТК СНВ-28 для обогрева кровель и площадок: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:url" content="https://elektromos.uz/Teplypolzdely/HTK/obogrevcrovliiplohadek/CNV-28" />
        <meta property="og:image" content="/images/seris/СНV-28.webp" />
        <meta name="twitter:card" content="summary_large_image" />
      </Head>
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-3xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Обогрев желобов и водостоков
...
<nav className="flex flex-wrap items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-400">
          ...
          <span>/</span>
          <span className="text-white">Обогрев желобов и водостоков
