...
export default function CN10Page() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это секция нагревательная СН-10
          if (!name.includes('секция нагревательная сн-10')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <Head>
        <title>СН-10 Специальный греющий кабель, ЧТК | Elektromos</title>
        <meta name="description" content="Купить  ЧТК СН-10 для обогрева холодильных камер: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:title" content="СН-10 — купить специальный греющий кабель | Elektromos" />
        <meta property="og:description" content="Купить ЧТК СН-10 для обогрева холодильных камер: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:url" content="https://elektromos.uz/Teplypolzdely/HTK/speyalnigreyhikabel/CN-10" />
        <meta property="og:image" content="/images/seris/CH-10.webp" />
        <meta name="twitter:card" content="summary_large_image" />
      </Head>
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-3xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Обогреве холодильных камер
</h2>
<nav className="flex flex-wrap items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-400">
          ...
          <span>/</span>
          <span className="text-white">Обогреве холодильных камер
