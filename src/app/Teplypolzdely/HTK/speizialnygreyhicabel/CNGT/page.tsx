...
export default function CNGTPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это секция нагревательная СНГТ
          if (!name.includes('секция нагревательная снгт')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <Head>
        <title>СНГТ Специальный греющий кабель, ЧТК | Elektromos</title>
        <meta name="description" content="Купить  ЧТК СНГТ для обогрева грунта в теплице: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:title" content="СНГТ — купить специальный греющий кабель | Elektromos" />
        <meta property="og:description" content="Купить ЧТК СНГТ для обогрева грунта в теплице: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:url" content="https://elektromos.uz/Teplypolzdely/HTK/speyalnigreyhikabel/CNGT" />
        <meta property="og:image" content="/images/seris/СНГТ.webp" />
        <meta name="twitter:card" content="summary_large_image" />
      </Head>
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-3xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">
