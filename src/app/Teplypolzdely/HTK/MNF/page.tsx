    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это фольгомат
          if (!name.includes('фольгомат')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <Head>
        ...
        <meta name="description" content="Купить  ЧТК МНФ для тёплого пола: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:title" content="МНФ — купить теплый пол под плитку | Elektromos" />
        <meta property="og:description" content="Купить ЧТК МНФ для тёплого пола: электронные, с датчиком пола, надежные решения для точного поддержания температуры." />
        <meta property="og:url" content="https://elektromos.uz/Teplypolzdely/HTK/MND" />
        <meta property="og:image" content="/images/seris/МНФ.webp" />
        <meta name="twitter:card" content="summary_large_image" />
      </Head>
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-3xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Мат нагревательный фольгированный под ламинат кварцвинил паркетнную доску
...
<nav className="flex flex-wrap items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-400">
          ...
          <span>/</span>
          <span className="text-white">MNF - Фольгомат</span>
        </nav>
        </div>
