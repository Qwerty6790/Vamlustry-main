...
export default function TermostatPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          if (!name.includes('терморегулятор')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-3xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Терморегулятор</h2>
          ...
          <span>/</span>
          <Link href="/Teplypolzdely/Werkel" className="hover:text-white transition-colors">Werkel</Link>
          <span>/</span>
          <span className="text-white">Терморегулятор</span>
        </nav>
