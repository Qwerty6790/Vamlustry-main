...
export default function SmokyMattePage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          if (!name.includes('alumax')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <Head>
        <title>Рамки Werkel - Alumax | Elektromos</title>
        <meta name="description" content="Рамки Werkel,серия Alumax. Особенности, характеристики, применение в интерьере.Купить в Москве Alumax ,Купить Розетки и выключатели" />
      </Head>
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-black mb-5">Alumax - Рамка</h2>
          ...
          <span>/</span>
          <span className="text-black">Alumax - Рамка</span>
        </nav>
        </div>
