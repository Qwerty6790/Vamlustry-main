...
export default function A07WaveWhitePage() {
  ...
  const fetchProducts = async (page: number = 1) => {
    ...
    try {
      ...
      if (data && data.products) {
        // Фильтруем товары для A07 Wave Белый (матовый)
        const filteredProducts = data.products.filter((product: any) => {
          const article = product.article || '';
          const name = (product.name || '').toLowerCase();
          
          // Проверяем что это серия A07 и цвет "Белый" (код 33 - матовый)
          ...
          console.log(`Товар: ${product.name}, артикул: ${article}, A07: ${isA07}, белый: ${isWhiteColor}, Wave продукт: ${isWaveProduct}, не Natural: ${isNotNatural}`);
          
          return isA07 && isWhiteColor && isWaveProduct && isNotNatural;
        });
        
        console.log(`Найдено товаров A07-Wave-белый: ${filteredProducts.length} из ${data.products.length}`);
    ...
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        ...
        <div className="mb-8">
          ...
          <span>/</span>
          <span className="text-white">A07 Wave Матовый белый</span>
        </nav>
        </div>
