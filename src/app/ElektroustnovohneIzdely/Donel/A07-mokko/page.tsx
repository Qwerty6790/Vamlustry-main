...
export default function A07MokkoPage() {
    ...
    try {
      ...
      if (data && data.products) {
        const filteredProducts = data.products.filter((product: any) => {
          ...
          const isMokkoColor = article.endsWith('53') || name.includes('мокко') || name.includes('mokko');
          ...
          return isA07 && isMokkoColor && isNotWave && isNotNatural;
        });
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        {/* Заголовок секции */}
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Мокко</h2>
          ...
          <span>/</span>
          <span className="text-white">A07 Мокко</span>
        </nav>
        </div>
