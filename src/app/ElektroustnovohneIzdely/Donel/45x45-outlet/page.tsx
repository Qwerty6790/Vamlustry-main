...
export default function Donel45x45OutletPage() {
  ...
  const fetchProducts = async (page: number = 1) => {
    try {
      ...
      // Оптимизированная загрузка - ищем только товары с розетками 45x45
      for (let pageNum = 1; pageNum <= 3; pageNum++) {
        try {
          ...
          console.log(`[Розетки 45x45] Загружаем страницу ${pageNum} с поиском "розетка"...`);
          ...
          if (response.data && response.data.products && Array.isArray(response.data.products)) {
            ...
            console.log(`[Розетки 45x45] Страница ${pageNum}: +${pageProducts.length} товаров (всего: ${allProducts.length})`);
            
            // Если нашли достаточно товаров или это последняя страница
            if (pageProducts.length < params.limit || allProducts.length > 100) {
              console.log(`[Розетки 45x45] Остановка загрузки на странице ${pageNum}`);
              break;
            }
          } else {
            console.log(`[Розетки 45x45] Страница ${pageNum}: нет данных`);
            break;
          }
          ...
          console.error(`[Розетки 45x45] Ошибка страницы ${pageNum}:`, error);
          break;
        }
      }
      ...
      if (uniqueProducts.length > 0) {
        // Быстрая фильтрация только розеток 2К 45x45
        const filteredProducts = uniqueProducts.filter((product: any) => {
          const name = (product.name || '').toLowerCase();
          const category = (product.category || '').toLowerCase();
          
          // Быстрая проверка на розетки 2К
          const hasOutlet2K = name.includes('розетка') && 
                             (name.includes('2к') || name.includes('2к+з'));
          ...
          // Исключаем USB, RJ45, Keystone
          const notUSB = !name.includes('usb');
          const notRJ45 = !name.includes('rj45');
          const notKeystone = !name.includes('keystone');
          
          const isMatch = hasOutlet2K && has45x45 && notUSB && notRJ45 && notKeystone;
          
          // Логируем только подходящие товары
          if (isMatch) {
            console.log(`[Розетки 45x45] НАЙДЕН: "${product.name}"`);
          }
        ...
        console.log(`✅ Найдено товаров Розетки 45x45: ${filteredProducts.length}`);
        if (filteredProducts.length > 0) {
          ...
          console.log('❌ Товары Розетки 45x45 не найдены в результатах поиска');
          ...
          try {
            ...
            if (fallbackResponse.data && fallbackResponse.data.products) {
              ...
              // Ищем розетки 2К 45x45 среди первых товаров
              const fallbackFiltered = fallbackProducts.filter((product: any) => {
                const name = (product.name || '').toLowerCase();
                const hasOutlet2K = name.includes('розетка') && (name.includes('2к') || name.includes('2к+з'));
                const has45x45 = name.includes('45х45') || name.includes('45x45');
                const notUSB = !name.includes('usb');
                const notRJ45 = !name.includes('rj45');
                const notKeystone = !name.includes('keystone');
                return hasOutlet2K && has45x45 && notUSB && notRJ45 && notKeystone;
              });
              ...
              if (fallbackFiltered.length > 0) {
                ...
                setTotalPages(1);
                setCurrentPage(1);
                console.log(`✅ Fallback: найдено товаров Розетки 45x45: ${fallbackFiltered.length}`);
              } else {
                console.log('❌ Fallback: товары Розетки 45x45 не найдены');
              }
            }
          } catch (fallbackError) {
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-52" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Розетки серии 45x45</h2>
          ...
          <span>/</span>
          <span className="text-white">45x45 Розетки</span>
        </nav>
        </div>
