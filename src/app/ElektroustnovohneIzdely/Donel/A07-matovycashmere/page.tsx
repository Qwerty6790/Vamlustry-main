...
export default function A07CashmerePage() {
    ...
    try {
      ...
      if (data && data.products) {
        const filteredProducts = data.products.filter((product: any) => {
          ...
          const isCashmereColor = article.endsWith('35') || name.includes('кашемир') || name.includes('cashmere');
          ...
          return isA07 && isCashmereColor && isNotWave && isNotNatural;
        });
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Матовый кашемир</h2>
          <nav className="flex flex-wrap items-center gap-1 sm:gap-2 text-sm sm:text-base text-white">
          <Link href="/" className="hover:text-white transition-colors">Главная</Link><span>/</span>
          <Link href="/ElektroustnovohneIzdely" className="hover:text-white transition-colors">Электроустановочные изделия</Link><span>/</span>
          <Link href="/ElektroustnovohneIzdely/Donel" className="hover:text-white transition-colors">Donel</Link><span>/</span>
          <span className="text-white">A07 Матовый кашемир</span>
        </nav>
        </div>
