...
export default function A07WaveMokkoPage() {
  ...
  const fetchProducts = async (page: number = 1) => {
    ...
    try {
      ...
      if (data && data.products) {
        // Фильтруем товары для A07 Wave Мокко
        const filteredProducts = data.products.filter((product: any) => {
          const article = product.article || '';
          const name = (product.name || '').toLowerCase();
          
          // Проверяем что это серия A07 и цвет "Мокко" (код 53)
          const isA07 = name.includes('a07') || article.toLowerCase().includes('a07');
          const isMokkoColor = article.endsWith('53') || name.includes('мокко');
          ...
          console.log(`Товар: ${product.name}, артикул: ${article}, A07: ${isA07}, мокко: ${isMokkoColor}, Wave продукт: ${isWaveProduct}, не Natural: ${isNotNatural}`);
          
          return isA07 && isMokkoColor && isWaveProduct && isNotNatural;
        });
        
        console.log(`Найдено товаров A07-Wave-мокко: ${filteredProducts.length} из ${data.products.length}`);
    ...
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        ...
        <div className="mb-8">
          ...
          <span>/</span>
          <span className="text-white">A07 Wave Мокко</span>
        </nav>
        </div>
