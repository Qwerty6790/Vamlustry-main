...
export default function S08WhiteGlossPage() {
    ...
    try {
      ...
      if (data && data.products) {
        const filteredProducts = data.products.filter((product: any) => {
          ...
          const isGlossWhiteColor = article.endsWith('03') || name.includes('глянцевый белый') || name.includes('gloss white');
          
          return isS08 && isGlossWhiteColor;
        });
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Глянцевый белый</h2>
          ...
          <span>/</span>
          <span className="text-white">S08 Глянцевый белый</span>
        </nav>
        </div>
