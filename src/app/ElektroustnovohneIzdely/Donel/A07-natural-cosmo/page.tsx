...
export default function A07NaturalGoldPage() {
  ...
  const fetchProducts = async (page: number = 1) => {
    ...
    try {
      ...
      if (data && data.products) {
        // Фильтруем товары для A07 Natural
        const filteredProducts = data.products.filter((product: any) => {
          const article = product.article || '';
          const name = (product.name || '').toLowerCase();
          
          // Проверяем что это серия A07 и золотистый алюминий
          ...
          const isGoldAluminum = name.includes('космический') || name.includes('cosmo') || article.includes('CA');
          
          console.log(`Товар: ${product.name}, артикул: ${article}, A07: ${isA07}, Natural: ${isNatural}, космический: ${isGoldAluminum}`);
          
          return isA07 && isNatural && isGoldAluminum;
        });
        
        console.log(`Найдено товаров A07-Natural-космический: ${filteredProducts.length} из ${data.products.length}`);
    ...
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        ...
        <div className="mb-8">
          ...
          <span>/</span>
          <span className="text-white">A07 Natural Космический алюминий</span>
        </nav>
        </div>
