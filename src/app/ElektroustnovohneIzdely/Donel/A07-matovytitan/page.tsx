...
export default function A07TitanPage() {
  ...
  const fetchProducts = async (page: number = 1) => {
    ...
    try {
      ...
      if (data && data.products) {
        // Фильтруем товары по коду цвета "45" в конце артикула
        const filteredProducts = data.products.filter((product: any) => {
          const article = product.article || '';
          const name = (product.name || '').toLowerCase();
          
          // Проверяем что это серия A07 и цвет "Матовый титан" (код 45 или слово "титан")
          const isA07 = name.includes('a07') || article.toLowerCase().includes('a07');
          const isTitanColor = article.endsWith('45') || name.includes('титан') || name.includes('titan');
          ...
          console.log(`Товар: ${product.name}, артикул: ${article}, A07: ${isA07}, титан: ${isTitanColor}, не Wave: ${isNotWave}, не Natural: ${isNotNatural}`);
          
          return isA07 && isTitanColor && isNotWave && isNotNatural;
        });
        
        console.log(`Найдено товаров A07-титан: ${filteredProducts.length} из ${data.products.length}`);
    ...
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        ...
        <div className="mb-8">
          ...
          <span>/</span>
          <span className="text-white">A07 Матовый титан</span>
        </nav>
        </div>
