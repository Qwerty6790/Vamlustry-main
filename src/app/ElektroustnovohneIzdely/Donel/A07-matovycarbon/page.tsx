...
export default function A07CarbonPage() {
  ...
  const fetchProducts = async (page: number = 1) => {
    ...
    try {
      ...
      if (data && data.products) {
        // Фильтруем товары по коду цвета "30" в конце артикула
        const filteredProducts = data.products.filter((product: any) => {
          const article = product.article || '';
          const name = (product.name || '').toLowerCase();
          
          // Проверяем что это серия A07 и цвет "Матовый карбон" (код 30 или слово "карбон")
          const isA07 = name.includes('a07') || article.toLowerCase().includes('a07');
          const isCarbonColor = article.endsWith('30') || name.includes('карбон') || name.includes('carbon');
          ...
          console.log(`Товар: ${product.name}, артикул: ${article}, A07: ${isA07}, карбон: ${isCarbonColor}, не Wave: ${isNotWave}, не Natural: ${isNotNatural}`);
          
          return isA07 && isCarbonColor && isNotWave && isNotNatural;
        });
        
        console.log(`Найдено товаров A07-карбон: ${filteredProducts.length} из ${data.products.length}`);
    ...
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        ...
        <div className="mb-8">
          ...
          <span>/</span>
          <span className="text-white">A07 Матовый карбон</span>
        </nav>
        </div>
