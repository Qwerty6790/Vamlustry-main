...
export default function A07WhiteGlassPage() {
    ...
    try {
      ...
      if (data && data.products) {
        const filteredProducts = data.products.filter((product: any) => {
          ...
          const isWhiteGlass = name.includes('white') || name.includes('бел') || article.toLowerCase().includes('wg');
          return isA07 && isNatural && isWhiteGlass;
        });
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Белое стекло</h2>
          <nav className="flex flex-wrap items-center gap-1 sm:gap-2 text-sm sm:text-base text-white">
            ...
            <span>/</span>
            <span className="text-white">A07 Natural Белое стекло</span>
          </nav>
        </div>
