...
export default function S08IvoryPage() {
    ...
    try {
      ...
      if (data && data.products) {
        const filteredProducts = data.products.filter((product: any) => {
          ...
          const isIvoryColor = article.endsWith('55') || name.includes('слоновая кость') || name.includes('ivory');
          
          return isS08 && isIvoryColor;
        });
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Слоновая кость</h2>
          ...
          <span>/</span>
          <span className="text-white">S08 Слоновая кость</span>
        </nav>
        </div>
