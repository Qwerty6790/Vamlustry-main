...
export default function A07ChampagnePage() {
  ...
  const fetchProducts = async (page: number = 1) => {
    try {
      ...
      if (data && data.products) {
        // Фильтруем товары по коду цвета "51" в конце артикула
        const filteredProducts = data.products.filter((product: any) => {
          const article = product.article || '';
          const name = (product.name || '').toLowerCase();
          
          // Проверяем что это серия A07 и цвет "Шампань" (код 51 или слово "шампань")
          ...
          console.log(`Товар: ${product.name}, артикул: ${article}, A07: ${isA07}, шампань: ${isChampagneColor}, не Wave: ${isNotWave}, не Natural: ${isNotNatural}`);
          
          return isA07 && isChampagneColor && isNotWave && isNotNatural;
        });
        
        console.log(`Найдено товаров A07-шампань: ${filteredProducts.length} из ${data.products.length}`);
...
<Head>
        <title>A07  - Шампань| Elektromos</title>
        <meta name="description" content="Серии A07 шампань. Особенности, характеристики, применение в интерьере.Купить в Москве A07" />
      </Head>
      {/* Основное содержимое */}
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        ...
        <div>
          ...
          <span>/</span>
          <span className="text-white">A07 Шампань</span>
        </nav>
        </div>
