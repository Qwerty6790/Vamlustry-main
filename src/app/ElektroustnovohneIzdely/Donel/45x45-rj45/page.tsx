...
export default function Donel45x45RJ45Page() {
  ...
  const fetchProducts = async (page: number = 1) => {
    try {
      ...
      // Оптимизированная загрузка - ищем только товары с RJ45 45x45
      for (let pageNum = 1; pageNum <= 3; pageNum++) {
        try {
          ...
          console.log(`[RJ45 45x45] Загружаем страницу ${pageNum} с поиском "rj45"...`);
          ...
          if (response.data && response.data.products && Array.isArray(response.data.products)) {
            ...
            console.log(`[RJ45 45x45] Страница ${pageNum}: +${pageProducts.length} товаров (всего: ${allProducts.length})`);
            
            // Если нашли достаточно товаров или это последняя страница
            if (pageProducts.length < params.limit || allProducts.length > 100) {
              console.log(`[RJ45 45x45] Остановка загрузки на странице ${pageNum}`);
              break;
            }
          } else {
            console.log(`[RJ45 45x45] Страница ${pageNum}: нет данных`);
            break;
          }
          ...
          console.error(`[RJ45 45x45] Ошибка страницы ${pageNum}:`, error);
          break;
        }
      }
      ...
      if (uniqueProducts.length > 0) {
        // Быстрая фильтрация только RJ45 розеток 45x45
        const filteredProducts = uniqueProducts.filter((product: any) => {
          const name = (product.name || '').toLowerCase();
          const category = (product.category || '').toLowerCase();
          
          // Быстрая проверка на RJ45 розетки
          const hasRJ45 = name.includes('rj45') || 
                         name.includes('rj-45') ||
                         category.includes('rj45') ||
                         name.includes('ethernet') ||
                         (name.includes('розетка') && name.includes('dec'));
          ...
          const isMatch = hasRJ45 && has45x45;
          
          // Логируем только подходящие товары
          if (isMatch) {
            console.log(`[RJ45 45x45] НАЙДЕН: "${product.name}"`);
          }
        ...
        console.log(`✅ Найдено товаров RJ45 45x45: ${filteredProducts.length}`);
        if (filteredProducts.length > 0) {
          ...
          console.log('❌ Товары RJ45 45x45 не найдены в результатах поиска');
          ...
          try {
            ...
            if (fallbackResponse.data && fallbackResponse.data.products) {
              ...
              // Ищем RJ45 розетки 45x45 среди первых товаров
              const fallbackFiltered = fallbackProducts.filter((product: any) => {
                const name = (product.name || '').toLowerCase();
                const hasRJ45 = name.includes('rj45') || name.includes('rj-45') || name.includes('ethernet');
                const has45x45 = name.includes('45х45') || name.includes('45x45');
                return hasRJ45 && has45x45;
              });
              ...
              if (fallbackFiltered.length > 0) {
                ...
                setTotalPages(1);
                setCurrentPage(1);
                console.log(`✅ Fallback: найдено товаров RJ45 45x45: ${fallbackFiltered.length}`);
              } else {
                console.log('❌ Fallback: товары RJ45 45x45 не найдены');
              }
            }
          } catch (fallbackError) {
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-52" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">RJ45 розетки серии 45x45</h2>
          ...
          <span>/</span>
          <span className="text-white">45x45 RJ45</span>
        </nav>
        </div>
