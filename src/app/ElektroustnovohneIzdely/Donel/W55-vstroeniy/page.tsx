...
export default function DonelW55VstroennyyPage() {
    ...
    try {
      ...
      for (let pageNum = 1; pageNum <= 5; pageNum++) {
        try {
          const params = {
            ...
            name: 'для скрытой установки', // Точное название категории как в каталоге
            inStock: 'true'
          };
      ...
      if (uniqueProducts.length > 0) {
        ...
        const filteredProducts = uniqueProducts.filter((product: any) => {
          ...
          // Проверяем что это встроенный монтаж (разные варианты написания)
          const isVstroennyy = name.includes('для скрытой установки') ||
                               name.includes('для скрытой установки') ||
                               name.includes('для скрытой установки') ||
                               name.includes('для скрытой установки') ||
                               name.includes('для скрытой установки') ||
                               category.includes('для скрытой установки') ||
                               category.includes('для скрытой установки') ||
                               (hasW55 && !name.includes('накладной') && !name.includes('накладной') && !name.includes('IP55'));
          
          return hasW55 && isVstroennyy;
        });
        ...
        console.log('Товары Встроенный монтаж W55:', filteredProducts.length);
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Встроенный монтаж</h2>
          ...
          <span>/</span>
          <span className="text-white">W55 Встроенный монтаж</span>
        </nav>
        </div>
