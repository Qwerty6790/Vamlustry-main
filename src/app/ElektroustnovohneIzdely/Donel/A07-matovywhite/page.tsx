...
export default function A07WhiteMattePage() {
  ...
  const fetchProducts = async (page: number = 1) => {
    ...
    try {
      ...
      if (data && data.products) {
        // Фильтруем товары по коду цвета "33" в конце артикула
        const filteredProducts = data.products.filter((product: any) => {
          const article = product.article || '';
          const name = (product.name || '').toLowerCase();
          
          // Проверяем что это серия A07 и цвет "Матовый белый" (код 33 или слово "белый")
          const isA07 = name.includes('a07') || article.toLowerCase().includes('a07');
          const isWhiteColor = article.endsWith('33') || (name.includes('белый') && name.includes('матовый'));
          ...
          console.log(`Товар: ${product.name}, артикул: ${article}, A07: ${isA07}, белый: ${isWhiteColor}, не Wave: ${isNotWave}, не Natural: ${isNotNatural}`);
          
          return isA07 && isWhiteColor && isNotWave && isNotNatural;
        });
        
        console.log(`Найдено товаров A07-белый: ${filteredProducts.length} из ${data.products.length}`);
    ...
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        ...
        <div className="mb-8">
          ...
          <span>/</span>
          <span className="text-white">A07 Матовый белый</span>
        </nav>
        </div>
