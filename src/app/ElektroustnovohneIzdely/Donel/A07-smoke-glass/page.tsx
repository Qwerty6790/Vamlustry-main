...
export default function A07SmokeGlassPage() {
    ...
    try {
      ...
      if (data && data.products) {
        const filteredProducts = data.products.filter((product: any) => {
          ...
          const isSmokeGlass = name.includes('smoke') || name.includes('дым') || article.toLowerCase().includes('sg');
          return isA07 && isNatural && isSmokeGlass;
        });
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Дымчатое стекло</h2>
          <nav className="flex flex-wrap items-center gap-1 sm:gap-2 text-sm sm:text-base text-white">
            ...
            <span>/</span>
            <span className="text-white">A07 Natural Дымчатое стекло</span>
          </nav>
        </div>
