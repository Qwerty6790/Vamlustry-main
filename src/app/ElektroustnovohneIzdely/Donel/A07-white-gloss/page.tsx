...
export default function A07WhiteGlossPage() {
  ...
  const fetchProducts = async (page: number = 1) => {
    ...
    try {
      ...
      if (data && data.products) {
        // Фильтруем товары по коду цвета "03" в конце артикула
        const filteredProducts = data.products.filter((product: any) => {
          const article = product.article || '';
          const name = (product.name || '').toLowerCase();
          
          // Проверяем что это серия A07 и цвет "Глянцевый белый" (код 03 или слово "глянцевый")
          const isA07 = name.includes('a07') || article.toLowerCase().includes('a07');
          const isGlossWhiteColor = article.endsWith('03') || (name.includes('белый') && name.includes('глянцевый'));
          ...
          console.log(`Товар: ${product.name}, артикул: ${article}, A07: ${isA07}, глянц белый: ${isGlossWhiteColor}, не Wave: ${isNotWave}, не Natural: ${isNotNatural}`);
          
          return isA07 && isGlossWhiteColor && isNotWave && isNotNatural;
        });
        
        console.log(`Найдено товаров A07-глянц белый: ${filteredProducts.length} из ${data.products.length}`);
    ...
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        ...
        <div className="mb-8">
          ...
          <span>/</span>
          <span className="text-white">A07 Глянцевый белый</span>
        </nav>
        </div>
