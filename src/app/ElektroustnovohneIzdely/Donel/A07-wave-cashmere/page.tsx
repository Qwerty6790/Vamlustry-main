...
export default function A07WaveCashmerePage() {
  ...
  const fetchProducts = async (page: number = 1) => {
    ...
    try {
      ...
      if (data && data.products) {
        // Фильтруем товары для A07 Wave Кашемир
        const filteredProducts = data.products.filter((product: any) => {
          const article = product.article || '';
          const name = (product.name || '').toLowerCase();
          
          // Проверяем что это серия A07 и цвет "Кашемир" (код 35)
          const isA07 = name.includes('a07') || article.toLowerCase().includes('a07');
          const isCashmereColor = article.endsWith('35') || name.includes('кашемир');
          ...
          console.log(`Товар: ${product.name}, артикул: ${article}, A07: ${isA07}, кашемир: ${isCashmereColor}, Wave продукт: ${isWaveProduct}, не Natural: ${isNotNatural}`);
          
          return isA07 && isCashmereColor && isWaveProduct && isNotNatural;
        });
        
        console.log(`Найдено товаров A07-Wave-кашемир: ${filteredProducts.length} из ${data.products.length}`);
 ...
 <Head>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-44" style={{ maxWidth: '88rem' }}>
        ...
        <div className="mb-8">
          ...
          <span>/</span>
          <span className="text-white">A07 Wave Матовый кашемир</span>
        </nav>
        </div>
