...
// --- КОМПОНЕНТ КОЛЛЕКЦИИ (БЫВШИЕ "ПОХОЖИЕ ТОВАРЫ") ---
          ...
          similar.map((product) => {
             ...
             return (
               <Link 
                 ...
                 className="w-20 h-20 bg-[#d9dbdc] rounded-2xl overflow-hidden flex items-center justify-center flex-shrink-0 hover:ring-2 hover:ring-black transition-all"
               >
                 {imgUrl ? (
...
const ProductDetail: React.FC<ProductDetailProps> = ({ product: initialProduct }) => {
  ...
  const imagesFromProduct = Array.isArray(product.imageAddresses) ? product.imageAddresses : typeof product.imageAddresses === 'string' ? [product.imageAddresses] : Array.isArray(product.imageAddress) ? product.imageAddress : typeof product.imageAddress === 'string' ? [product.imageAddress] : [];
  const mainImg = imagesFromProduct[0] ? getImageUrl(imagesFromProduct[0]) : null;
  const secondImg = imagesFromProduct[1] ? getImageUrl(imagesFromProduct[1]) : mainImg; // Fallback to main if no second

  return (
    <>
      <SEO 
        title={`${product.name} | ВамЛюстра`}
        description={`Купить ${product.name}. Цена: ${product.price} ₽.`}
        ...
        <main className="container mx-auto px-4 md:px-8 pt-28 pb-20 max-w-[1500px]">
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
            ...
            <div className="lg:w-[55%] xl:w-[60%] flex flex-col gap-4">
              ...
              <div className="flex flex-col sm:flex-row gap-4 h-[400px] sm:h-[500px] xl:h-[600px]">
                {/* Главное фото */}
                <div className="relative flex-1 bg-[#dcdcdc] rounded-[32px] overflow-hidden flex items-center justify-center p-8 group">
                  <div className="absolute top-4 right-4 bg-white/80 backdrop-blur-md px-4 py-2 rounded-full text-sm font-medium z-10 cursor-pointer hover:bg-white transition-colors">
                  ...
                  <button className="absolute bottom-4 right-4 bg-white/80 backdrop-blur-md px-4 py-2 rounded-full text-sm font-medium z-10 flex items-center gap-2 hover:bg-white transition-colors">
                    <Search size={16} /> Похожие
                  </button>
                </div>

                {/* Второе фото */}
                <div className="hidden sm:flex relative flex-1 bg-[#dcdcdc] rounded-[32px] overflow-hidden items-center justify-center p-8 group">
                  {secondImg && (
                    <img src={secondImg} alt={`${product.name} ракурс`} className="w-full h-full object-contain mix-blend-multiply group-hover:scale-105 transition-transform duration-500" />
                  )}
                </div>
              ...
              <div className="bg-[#f5f5f5] rounded-[32px] p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 overflow-hidden relative">
                 <div className="sm:max-w-[60%] z-10 relative">
                    <h3 className="text-2xl font-medium mb-2">Вид</h3>
                    <p className="text-gray-600 text-sm">
                      Загрузите фотографию места установки светильника и посмотрите, как он будет выглядеть в вашем пространстве — быстро, реалистично и без специальных гаджетов.
                    </p>
                 </div>
                 <div className="absolute right-0 top-0 bottom-0 w-[40%] opacity-40 sm:opacity-100 pointer-events-none flex items-center justify-end pr-4">
                    ...
                    <div className="w-32 h-32 border-2 border-gray-300 rounded-lg flex items-center justify-center bg-white/50 backdrop-blur-sm rotate-6">
                       <img src={mainImg || ''} className="w-20 h-20 object-contain mix-blend-multiply opacity-50" />
                    </div>
                 </div>
              </div>
            </div>
            ...
            <div className="lg:w-[45%] xl:w-[40%] flex flex-col">
              ...
              <div className="flex gap-6 border-b border-gray-200 mb-8 overflow-x-auto scrollbar-hide text-sm">
                ...
                <button className="pb-3 border-b-2 border-transparent text-gray-500 hover:text-black whitespace-nowrap">Инструкция</button>
                <button className="pb-3 border-b-2 border-transparent text-gray-500 hover:text-black whitespace-nowrap">Сделать умным</button>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                <div className="flex-1 flex items-start gap-4 border border-gray-200 rounded-2xl p-4">
                  ...
                  <div className="text-sm">Подарок в корзине при заказе от 30 000 ₽</div>
                </div>
                <div className="flex-1 flex items-start gap-4 border border-gray-200 rounded-2xl p-4">
                  <PenTool className="w-8 h-8 text-black shrink-0" strokeWidth={1.5} />
                  <div className="text-sm">Сертификат на монтажные работы при заказе от 30 000 ₽</div>
                </div>
              </div>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  try {
    const response = await fetch(`${BASE_URL}/api/product/${supplier}?productArticle=${article}`);
